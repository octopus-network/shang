...
fn should_make_http_call_and_parse_result() {
	...
	t.execute_with(|| {
		// when
		let val_set = Example::fetch_validator_set(b"octopus.testnet".to_vec(), 0, 1).ok();
		// then
...
fn encode_args_works() {
	let test_data = vec![
		(0u32, 0u32, Some(vec![101, 121, 74, 104, 99, 72, 66, 106, 97, 71, 70, 112, 98, 108, 57, 112, 90, 67, 73, 54, 77, 67, 119, 105, 99, 50, 86, 120, 88, 50, 53, 49, 98, 83, 73, 54, 77, 72, 48, 61])), // eyJhcHBjaGFpbl9pZCI6MCwic2VxX251bSI6MH0=
		(4294967295u32, 4294967295u32, Some(vec![101, 121, 74, 104, 99, 72, 66, 106, 97, 71, 70, 112, 98, 108, 57, 112, 90, 67, 73, 54, 78, 68, 73, 53, 78, 68, 107, 50, 78, 122, 73, 53, 78, 83, 119, 105, 99, 50, 86, 120, 88, 50, 53, 49, 98, 83, 73, 54, 78, 68, 73, 53, 78, 68, 107, 50, 78, 122, 73, 53, 78, 88, 48, 61])), // eyJhcHBjaGFpbl9pZCI6NDI5NDk2NzI5NSwic2VxX251bSI6NDI5NDk2NzI5NX0=
	];

	for (appchain_id, seq_num, expected) in test_data {
		assert_eq!(expected, Example::encode_args(appchain_id, seq_num));
	}
}
...
fn should_submit_unsigned_transaction_on_chain() {
	...
	t.execute_with(|| {
		// when
		Example::fetch_and_update_validator_set(1, 1).unwrap();
		// then
		...
		if let Call::Example(crate::Call::submit_validator_set(body, signature)) = tx.call {
			...
			let signature_valid = <ValidatorSetPayload<
				...
				<Test as frame_system::Config>::AccountId
					> as SignedPayload<Test>>::verify::<crypto::OctopusAuthId>(&payload, signature);
